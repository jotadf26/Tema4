...
* Estudiante: **Mauricio Morales Morales 997378**
* Estudiante: **Josue Flores Vargas B62696**
* Estudiante: **Oscar Quesada Muñoz B86214**
* Grupo: **P4.11**
...
```
    ...
    Duración de la simulación:  11.058401346206665
    2427 errores, para un BER de 0.0057.
    




    <matplotlib.image.AxesImage at 0x174881846a0>
...
def modulador_8_PSK(bits, fc, mpp):
    ...
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)  # mpp: muestras por período
    # Se definen la portadora cos y la portadora sen
    ...
    for i in range(0, N, 3): # Donde bit1 = bits[i], bit2 = bits[i+1], bit3 = bits[i+2]
        numero_binario = [bits[i],bits[i+1],bits[i+2]] #str(bits[i],bits[i+1],bits[i+2])
        numero_decimal = 0 # Aquí se guarda el valor decimal
        ...
        for posicion, digito_string in enumerate(numero_binario[::-1]):
            ...
            if numero_decimal == 0:
                ...
                A1 = 0.707
                A2 = -0.707
            elif numero_decimal == 6:
                A1 = 0.707
                A2 = 0.707
            elif numero_decimal == 7:
                A1 = 1
                A2 = 0
           
        senal_Tx[j *mpp: (j+1)*mpp] = A1 * portadora_cos + A2 * portadora_sin
        j += 1
    # 5. Calcular la potencia promedio de la señal modulada
    P_senal_Tx = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
...
def demodulador_8_PSK(senal_Rx, portadora_cos, portadora_sin, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema BPSK. El criterio
    de demodulación se basa en decodificación por 
    detección de energía.
    ...
    for i in range(N):
        ''' Se iguala la cantidad de iteraciones que puede correr el programa y la cantidad 
            de información que se puede recuperar '''
        if j+3 > N:  
            break   
    ...
    # Para asignar signos:
        if np.sum(producto_cos) >= 1:
            bits_Rx[j] = 1  # Para el bit 1
            
        if np.sum(producto_sin) == -1:
            bits_Rx[j] = 1  # Para el bit 1
     
        if np.sum(producto_sin) >= 0.707:
...
senal_Tx, Pm, portadora_cos, portadora_sin = modulador_8_PSK(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_8_PSK(senal_Rx, portadora_cos, portadora_sin, mpp)
...
```
    ...
    Duración de la simulación:  14.758596420288086
    111167 errores, para un BER de 0.2629.
...
```python
#se empieza por definir las variables y el tiempo
T = np.linspace(0, 0.1, 100)
N = 8
t = 100
t_final = 0.1

# Se crea un vector que contenga los valores posibles.
A = [-1, -0.707, 0, 0.707, 1]
...
for i in range(len(A)):
    s1 =  i * np.cos(2*(np.pi)*fc*T) + i * (np.sin(2*(np.pi)*fc*T))
    s2 = -i * np.cos(2*(np.pi)*fc*T) + i * (np.sin(2*(np.pi)*fc*T))
    s3 =  i * np.cos(2*(np.pi)*fc*T) + -i * (np.sin(2*(np.pi)*fc*T))
...
plt.plot(T, P, lw=4, color = 'red', label = 'Promedio de realizaciones')

VE = np.mean(senal_Tx)*T # Se calcula la media teórica
plt.plot(T, VE, '-.', lw=3, color = 'b', label='valor esperado teórico')
...
for n in range(N):
    for i, tau in enumerate(desplazamiento):
        corr[n, i] = np.correlate(s_t[n,:], np.roll(s_t[n,:], tau))/t
    plt.plot(taus, corr[n,:])

# Valor teórico de correlación donde la magnitud es:
